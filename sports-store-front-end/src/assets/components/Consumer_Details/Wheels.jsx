      ...
      >
        <Drawer.Items>
          <Wheel_Detail_Form
            ...
            handleUpdatedWheelsDetails={handleUpdatedWheelsDetails}
          />
        </Drawer.Items>
      </Drawer>
      <div className="mb-12">
        ...
        <div className="bg-white grid-col-[repeat(1,minmax(min-content, max-content))] ">
          ...
          <h3 className="ml-11 text-xl font-medium">
            Your Wheels[{registeredEheelsDetails.length}]
          </h3>

          <div className="bg-white">
            <div className="ml-11">
              <h3 className="px-6 font-semibold mt-5">
                Shipping Addresses [{registeredEheelsDetails.length}]
              </h3>
              <div
                className={`grid gap-6 grid-cols-[repeat(${
                  registeredEheelsDetails.length + 1
                },minmax(370px,1fr))] md:grid-cols-[repeat(2,minmax(100px,1fr))] md:grid-rows-1 overflow-x-auto p-10`}
              >
                {registeredEheelsDetails &&
                  registeredEheelsDetails.map((info) => (
                    <Wheel_info_Cards wheelInfo={info} key={info.purchaseId} />
                  ))}
